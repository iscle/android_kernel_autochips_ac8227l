...
#include <linux/clk.h>
struct clk *usb0main_clk;
struct clk *usb1main_clk;
struct clk *usb0mcu_clk;
struct clk *usb1mcu_clk;
...
static int mt_usb_dts_probe(struct platform_device *pdev)
{
...
#ifndef CONFIG_MTK_CLKMGR
	usb0main_clk = devm_clk_get(&pdev->dev, "usb0_main");
	if (IS_ERR(usb0main_clk)) {
		DBG(0, KERN_WARNING "cannot get usb0_main clock\n");
		return PTR_ERR(usb0main_clk);
	}
	DBG(0, KERN_WARNING "get usb0main_clk clock ok, prepare it\n");
	retval = clk_prepare(usb0main_clk);
	if (retval == 0) {
	...
	usb0mcu_clk = devm_clk_get(&pdev->dev, "usb0_mcu");
	if (IS_ERR(usb0mcu_clk)) {
		DBG(0, KERN_WARNING "cannot get usb0mcu_clk clock\n");
		return PTR_ERR(usb0mcu_clk);
	}
	DBG(0, KERN_WARNING "get usb0 clock ok, prepare it\n");
	retval = clk_prepare(usb0mcu_clk);
	if (retval == 0) {
		DBG(0, KERN_WARNING "prepare done\n");
	} else {
		DBG(0, KERN_WARNING "prepare fail\n");
	...
	usb1main_clk = devm_clk_get(&pdev->dev, "usb1_main");
	if (IS_ERR(usb1main_clk)) {
		DBG(0, KERN_WARNING "cannot get usb1_main clock\n");
		return PTR_ERR(usb1main_clk);
	}
	DBG(0, KERN_WARNING "get usb1_main clock ok, prepare it\n");
	retval = clk_prepare(usb1main_clk);
	if (retval == 0) {
		DBG(0, KERN_WARNING "prepare done\n");
	} else {
		DBG(0, KERN_WARNING "prepare fail\n");
	...
	usb1mcu_clk = devm_clk_get(&pdev->dev, "usb1_mcu");
		if (IS_ERR(usb1mcu_clk)) {
			DBG(0, KERN_WARNING "cannot get usb1mcu_clk clock\n");
			return PTR_ERR(usb1mcu_clk);
		}
		DBG(0, KERN_WARNING "get usb1 clock ok, prepare it\n");
		retval = clk_prepare(usb1mcu_clk);
		if (retval == 0) {
			DBG(0, KERN_WARNING "prepare done\n");
		} else {
			DBG(0, KERN_WARNING "prepare fail\n");
