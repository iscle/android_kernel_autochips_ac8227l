...
int get_ddr_type(void)
{
	...
	value = readl(DRAMC_BASE_ADDR + DRAMC_LPDDR2);

	if ((value >> 28) & 0x1)
		return LPDDR2;

	value = readl(DRAMC_BASE_ADDR + DRAMC_PADCTL4);
	if ((value >> 7) & 0x1) {
		if (readl(DRAMC_BASE_ADDR + DRAMC_CONF1) & 0x1)
			return DDR3_32;
		else
			return DDR3_16;
	}
	...
	value = readl(DRAMC_BASE_ADDR + DRAMC_ACTIM1);
...
DRIVER_ATTR(ddr_size, 0444, ddr_size_show, NULL);
/*
...
static ssize_t finetune_md_show(struct device_driver *driver, char *buf)
{
    ...
    /*TBD*/
		...
		sprintf(buf, "ERROR: unknown dram type!");    /*unknown dram type*/
	}
