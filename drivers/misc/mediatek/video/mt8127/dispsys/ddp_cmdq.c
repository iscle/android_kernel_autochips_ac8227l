								...
								loff_t *ppos)
{
	...
	int32_t	  execTime;
	int32_t	  totalTime;
	char *pBuffer = (char *)kmalloc(sizeof(char) * count, GFP_KERNEL);
	ssize_t size = 0;
	...
	for (; numRec > 0; numRec--, index++) {
		...
		length += snprintf(&pBuffer[length], count - length,
			"Idx: %3d, SCE: %d, PRI: %2d, REO: %d, SEC: %d, trigger[%d:%06d], Time[IRQ, Exec, Total]: ms[%3d, %3d, %3d]\n",
			index, pRecord->scenario, pRecord->priority, pRecord->reorder, pRecord->isSecure,
			(uint32_t)pRecord->trigger.tv_sec, (uint32_t)pRecord->trigger.tv_usec,
			IRQTime, execTime, totalTime);
	...
	/* *ppStart  = pPage + (offset - begin); */
	size = simple_read_from_buffer(pPage, count, ppos, pBuffer, strlen(pBuffer));
	kfree(pBuffer);
	return size;
...
uint32_t *cmdq_core_get_buffer_VA(void)
{
	return gCmdqContext.taskInfo[0].pVABase;
}
uint32_t cmdq_core_get_buffer_PA(void)
{
	return gCmdqContext.taskInfo[0].MVABase;
}
