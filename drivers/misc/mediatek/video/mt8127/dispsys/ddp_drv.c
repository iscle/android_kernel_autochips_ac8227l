...
/*DTS2016122808715,yuquan/ywx422261 added DSM for display,begin @20161214 */
#ifdef CONFIG_HUAWEI_DSM
#include <dsm/dsm_pub.h>
#endif
/*DTS2016122808715,yuquan/ywx422261 added DSM for display,end @20161214 */
...
unsigned int disp_log_level = 0;//DISPLOG_MSG;/*DISPLOG_FENCE;(DISPLOG_ALL & (~DISPLOG_IRQ));*/
#define REMOVE_REDUNDANCE_CODE_ON_MT8127
/* device and driver */
...
/*DTS2016122808715,yuquan/ywx422261 added DSM for display,begin @20161214 */
#ifdef CONFIG_HUAWEI_DSM
#define CLIENT_NAME_LCD "dsm_lcd"
static struct dsm_dev dsm_lcd_dev = {
	.name = CLIENT_NAME_LCD,// dsm client name
	.fops = NULL,// options
	.buff_size = DSM_SENSOR_BUF_MAX,// buffer size
};
struct dsm_client *dsm_lcd_dclient = NULL;
int dsm_disp_report_err(int errno)
{
	int size = 0;

	if(!dsm_lcd_dclient)
	{
		DISP_ERR("%s: dsm_lcd_dclient is null\n", __func__);
		return -EFAULT;
	}
	if(dsm_client_ocuppy(dsm_lcd_dclient))
	{
		/* buffer is busy */
		DISP_ERR("%s: buffer is busy!, errno = %d\n", __func__,errno);
		return -EBUSY;
	}
	size = dsm_client_record(dsm_lcd_dclient,"lcd mipi cmd fail,error=%d\n",errno);
	dsm_client_notify(dsm_lcd_dclient, errno);
	return size;
}
#endif
/*DTS2016122808715,yuquan/ywx422261 added DSM for display,end @20161214 */
...
/* internal function */
#ifndef REMOVE_REDUNDANCE_CODE_ON_MT8127
...
/*< DTS2017030605275 xingbin/xwx427571 20170313 begin*/
#ifdef DISP_IOCTL_READ_REG_TABLE_SUPPORT
	DISP_READ_REG_TABLE rTableParams;
#endif
/* TS2017030605275 xingbin/xwx427571 20170313 end >*/
	unsigned int ret = 0;
	unsigned int value;
...
#endif
		DISP_DBG("%s,DISP_IOCTL_READ_REG_TABLE selected\n",__func__);
/*DTS2017030605275 xingbin/xwx427571 20170313 end >*/
		break;
...
/*DTS2016122808715,yuquan/ywx422261 added DSM for display,begin @20161214 */
#ifdef CONFIG_HUAWEI_DSM
	dsm_lcd_dclient = dsm_register_client(&dsm_lcd_dev);
	if(dsm_lcd_dclient==NULL)
	{
		DISP_ERR("dsm_register_client disp_dsm_client failed\n");
	}
#endif
/*DTS2016122808715,yuquan/ywx422261 added DSM for display,end @20161214 */
	return 0;
...
/*DTS2016122808715,yuquan/ywx422261 added DSM for display,begin @20161214 */
	...
	if (dsm_lcd_dclient)
	{
		dsm_unregister_client(dsm_lcd_dclient,&dsm_lcd_dev);
		dsm_lcd_dclient = NULL;
	}
#endif
/*DTS2016122808715,yuquan/ywx422261 added DSM for display,end @20161214 */
