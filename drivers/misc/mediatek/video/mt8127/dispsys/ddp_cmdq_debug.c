...
uint32_t *cmdq_core_get_pc(const TaskStruct *pTask, uint32_t thread, uint32_t insts[4])
{
	...
	if (((uint8_t *)pTask->pVABase <= pInst) && (pInst <= (uint8_t *)pTask->pCMDEnd)) {
		...
		/* invalid PC address */
		uint32_t *VA, PA;
		VA = cmdq_core_get_buffer_VA();
		PA = cmdq_core_get_buffer_PA();
		CMDQ_ERR("currentPC[0x%x] CurrentTaskMVA[0x%x] CurrentTaskVA[%p] BeginTaskMVA[0x%x] BeginTaskVA[%p]\n",
			currPC, pTask->MVABase, pTask->pVABase, PA, VA);

		if ((currPC >= PA) && (currPC < (PA + CMDQ_MAX_DMA_BUF_SIZE))) {
			pInst = (uint8_t *)VA + (currPC - PA);
			insts[2] = DISP_REG_GET(pInst + 0);
			insts[3] = DISP_REG_GET(pInst + 4);
		} else
			return NULL;
