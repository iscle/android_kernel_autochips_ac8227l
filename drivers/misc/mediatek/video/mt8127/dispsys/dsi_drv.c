...
extern int dsm_disp_report_err(int errno);
#endif
...
static void _WaitForEngineNotBusy(void)
{
	...
	if (in_interrupt()) {
		...
		while (_IsEngineBusy()) {
			msleep(20);
			if (--timeOut < 0) {
				/*DTS2016122808715,yuquan/ywx422261 added DSM for lcd,begin @20161214 */
				#ifdef CONFIG_HUAWEI_DSM
					dsm_disp_report_err(DSM_LCD_LDI_UNDERFLOW_ERROR_NO);
				#endif
				/*DTS2016122808715,yuquan/ywx422261 added DSM for lcd,end @20161214 */
...
#define MIPI_HS_TRAL_ADJUST            0x03
...
#define NS_TO_CYCLE(n, c)   ((n) / (c))
	...
	timcon0.HS_TRAIL = ((hs_trail_m > hs_trail_n) ? hs_trail_m : hs_trail_n) + MIPI_HS_TRAL_ADJUST;
	/*< DTS2017010406947 xingbin/xwx427571 20170104 end */
