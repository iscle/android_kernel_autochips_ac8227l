...
{
		/*<DTS2016040704823   yangfei/ywx353749 20160719 begin */
    ...
    if((bl_state==1)&&(level >0))
    {
      bl_state=0;
    LOG_JANK_D(JLID_KERNEL_LCD_BACKLIGHT_ON,"%s,%d", "JLID_KERNEL_LCD_BACKLIGHT_ON",level);
		/*<DTS2016040704823   yangfei/ywx353749 20160719 end */
    }
#endif
    /* DTS2016040704823   tanfuhai/twx343282 20160504 end */
...
static int mtkfb_pan_display_impl(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	static int need_skip_black = 1;

	if (need_skip_black > 0 && strcmp(current->comm, "surfaceflinger") == 0) {
		need_skip_black--;
		return 0;
	}
	...
	/*< DTS2017031706118 xingbin/xwx427571 20170321 begin */
	if (down_interruptible(&sem_early_suspend)) {
		DISP_ERR("can't get semaphore(sem_early_suspend) in mtkfb_pan_display_impl()\n");
		return -ERESTARTSYS;
	}

	if (is_early_suspended) {
		DISP_FB("mtkfb_pan_display_impl is_early_suspend\n");
		up(&sem_early_suspend);
		return -ERESTARTSYS;
	}
	/* DTS2017031706118 xingbin/xwx427571 20170321 end >*/
	if (down_interruptible(&sem_flipping)) {
...
#define CMDLINE_LCM_HEADER_LEN        6
...
#define	INTEGER_INIT_ZERO		0
static ssize_t lcd_mipi_write_store(struct device* dev,
        struct device_attribute* attr, const char* buf, size_t count)
{
	ssize_t ret = INTEGER_INIT_ZERO;
	u32  mipi_reg = INTEGER_INIT_ZERO;
	u32  mipi_reg_val = INTEGER_INIT_ZERO;

	if (is_early_suspended) {
		return sprintf((char*)buf, "fail\n");
	}
	ret = sscanf(buf, "0x%x 0x%x",&mipi_reg,&mipi_reg_val);
	if (ret != 2)
		return -EINVAL;

	lcd_mipi_reg_write(mipi_reg,mipi_reg_val);

	return count;
}
static ssize_t lcd_mipi_write_show(struct device* dev,
        struct device_attribute* attr, char* buf)
{
    return 0;
}
static DEVICE_ATTR(lcd_mipi_write, S_IRUGO | S_IWUSR, lcd_mipi_write_show, lcd_mipi_write_store);
static struct attribute* lcd_fb_attrs[] =
{
    &dev_attr_lcd_mipi_write.attr,
    NULL,
};
struct attribute_group lcd_fb_attr_group =
{
    .attrs = lcd_fb_attrs,
};
int lcd_fb_create_sysfs(struct kobject* obj)
{
    int rc;

    rc = sysfs_create_group(obj, &lcd_fb_attr_group);
...
/*< DTS2017011300079 xingbin/xwx427571 20170108 end */
...
static int mtkfb_probe(struct platform_device *pdev)
{
	...
	lcd_fb_create_sysfs(&fbi->dev->kobj);
