...
static inline int m4u_port_2_smi_port(M4U_PORT_ID_ENUM port)
{
	...
	local_port = m4u_port_2_larb_port(port);
...
{				/* native */
	...
	m4u_base = gM4UBaseAddr[m4u_port_2_m4u_id(PortID)];
	eModuleID = m4u_port_2_module(PortID);
...
static int m4u_pm_suspend_noirq(struct device *device)
{
	struct platform_device *pdev = to_platform_device(device);
	...
	return m4u_clock_off();

}

static int m4u_pm_resume_noirq(struct device *device)
{
	struct platform_device *pdev = to_platform_device(device);
...
#define m4u_pm_suspend_noirq  NULL
#define m4u_pm_resume_noirq  NULL
...
const struct dev_pm_ops m4u_pm_ops = {
	...
	.suspend_noirq = m4u_pm_suspend_noirq,
	.resume_noirq = m4u_pm_resume_noirq,
