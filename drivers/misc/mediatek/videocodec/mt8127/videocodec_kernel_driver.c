...
static long vcodec_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case VCODEC_SET_THREAD_ID:
		MODULE_MFV_LOGD("[8127] VCODEC_SET_THREAD_ID [EMPTY] + tid = %d\n", current->pid);
	...
	case VCODEC_ALLOC_NON_CACHE_BUFFER:
		{
			MODULE_MFV_LOGD("[8127][M4U]! %s,line:%d VCODEC_ALLOC_NON_CACHE_BUFFER + tid = %d\n",
			...
			MODULE_MFV_LOGD("[8127][M4U]! VCODEC_ALLOC_NON_CACHE_BUFFER - tid = %d\n",
				 current->pid);
		}
		break;
	...
	case VCODEC_INC_ENC_EMI_USER:
		...
		gu4EncEMICounter++;
		MODULE_MFV_LOGD("ENC_EMI_USER = %d\n", gu4EncEMICounter);
		user_data_addr = (VAL_UINT8_T *) arg;
			...
			if (rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
				...
				while (bLockedHW == VAL_FALSE) {
					...
					if (EncHWLockEvent.u4TimeoutMs == 1) {
						MODULE_MFV_LOGD("[NOT ERROR][VCODEC_LOCKHW] ENC First Use HW %d!!\n", rHWLock.eDriverType);
						FirstUseEncHW = 1;
					} else {
						FirstUseEncHW = 0;
					}
	...
	case VCODEC_INITHWLOCK:
		{
			MODULE_MFV_LOGD("[8127] VCODEC_INITHWLOCK [EMPTY] + - tid = %d\n", current->pid);
	...
	case VCODEC_DEINITHWLOCK:
		{
			MODULE_MFV_LOGD("[8127] VCODEC_DEINITHWLOCK [EMPTY] + - tid = %d\n", current->pid);
	...
	case VCODEC_SET_CPU_OPP_LIMIT:
		{
			MODULE_MFV_LOGD("[8127] VCODEC_SET_CPU_OPP_LIMIT [EMPTY] + - tid = %d\n",
				 current->pid);
			user_data_addr = (VAL_UINT8_T *) arg;
			ret =
			...
			MODULE_MFV_LOGD("+VCODEC_SET_CPU_OPP_LIMIT (%d, %d, %d), tid = %d\n",
			...
			MODULE_MFV_LOGD("-VCODEC_SET_CPU_OPP_LIMIT tid = %d, ret = %lu\n", current->pid,
				 ret);
			MODULE_MFV_LOGD("[8127] VCODEC_SET_CPU_OPP_LIMIT [EMPTY] - - tid = %d\n",
				 current->pid);
		}
...
static int vcodec_open(struct inode *inode, struct file *file)
{
	...
	MODULE_MFV_LOGD("vcodec_open pid = %d, MT8127Driver_Open_Count %d\n", current->pid,
		 MT8127Driver_Open_Count);
...
static int venc_hw_reset(int type)
{
	...
	MODULE_MFV_LOGD("Start VENC HW Reset");
	...
	if (VAL_RESULT_INVALID_ISR == eValRet || gu4HwVencIrqStatus != VENC_IRQ_STATUS_PAUSE) {
		...
		MODULE_MFV_LOGD("irq_status 0x%x", uValue);
		VDO_HW_WRITE(KVA_VENC_SW_PAUSE, 0);
		VDO_HW_WRITE(KVA_VENC_SW_HRST_N, 0);
		uValue = VDO_HW_READ(KVA_VENC_SW_HRST_N);
		MODULE_MFV_LOGD("3 HRST = %d, isr = 0x%x", uValue, gu4HwVencIrqStatus);
	} else {		/* step 4 */
		VDO_HW_WRITE(KVA_VENC_SW_HRST_N, 0);
		uValue = gu4HwVencIrqStatus;
		VDO_HW_WRITE(KVA_VENC_SW_PAUSE, 0);
		MODULE_MFV_LOGD("4 HRST = %d, isr = 0x%x", uValue, gu4HwVencIrqStatus);
	}

	VDO_HW_WRITE(KVA_VENC_SW_HRST_N, 1);
	uValue = VDO_HW_READ(KVA_VENC_SW_HRST_N);
	MODULE_MFV_LOGD("HRST = %d", uValue);
...
static int vcodec_flush(struct file *file, fl_owner_t id)
{
	...
	MODULE_MFV_LOGD("vcodec_flush pid = %d, MT8127Driver_Open_Count %d\n",
...
static int vcodec_release(struct inode *inode, struct file *file)
{
	...
	if (MT8127Driver_Open_Count == 0) {
		...
		if (grVcodecEncHWLock.eDriverType == VAL_DRIVER_TYPE_H264_ENC) {
			...
			venc_power_off();
			MODULE_MFV_LOGD("Clean venc lock\n");
		}
		mutex_unlock(&VencHWLock);
		...
		mutex_lock(&L2CLock);
		if (gu4L2CCounter != 0) {
			MODULE_MFV_LOGD("vcodec_flush pid = %d, L2 user = %d, force restore L2 settings\n",
...
{
	mutex_lock(&PWRLock);
	MODULE_MFV_LOGD("vcodec_early_suspend, tid = %d, PWR_USER = %d\n", current->pid, gu4PWRCounter);
	mutex_unlock(&PWRLock);
/*
    if (gu4PWRCounter != 0)
    {
...
{
	mutex_lock(&PWRLock);
	MODULE_MFV_LOGD("vcodec_late_resume, tid = %d, PWR_USER = %d\n", current->pid, gu4PWRCounter);
	mutex_unlock(&PWRLock);
/*
    if (gu4PWRCounter != 0)
    {
...
static int vdec_fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	MODULE_MFV_LOGD("vdec_fb_notifier_callback %ld, %d\n", event , FB_EVENT_BLANK);
	...
	}
	{
		...
		MODULE_MFV_LOGD
		    ("[DeviceTree] %s,line:%d KVA_VENC_BASE(0x%lx), KVA_VDEC_BASE(0x%lx), KVA_VDEC_GCON_BASE(0x%lx)",
		      __FUNCTION__, __LINE__, KVA_VENC_BASE, KVA_VDEC_BASE, KVA_VDEC_GCON_BASE);
